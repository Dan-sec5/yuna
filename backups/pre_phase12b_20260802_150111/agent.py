...
class YunaAgent:
    ...
    def _ejecutar_tool_loop(
        ...
        """
        Ejecuta un ciclo multi-step de tool calling.

        Flujo:
            Ollama -> tool call -> executor -> resultado -> Ollama

        El ciclo termina cuando:
        - Ollama no solicita más herramientas.
        - Se alcanza max_steps.
        - Ocurre un error grave.

        Retorna:
            (respuesta_ollama, resultados_tools, tool_names)
        """
        resultados_tools = []
        tool_names = []
        ...
        for step in range(max_steps):
            ...
            logger.info(
                f"Tool calls detectados en paso {step + 1}: "
                f"{len(tool_calls)}"
            )
            ...
            messages.extend(mensajes_resultado)

        logger.warning(
            f"Tool loop alcanzó máximo de {max_steps} pasos"
        )
