...
def abrir_terminal(comando):
    script = f'tell application "Terminal" to do script "{comando}"'
...
class YunaAvatar:
    ...
    def cerrar_yuna(self):
        ...
        bitacora = os.path.expanduser(
            "~/yuna/bitacora.txt"
        )

        historial_dir = os.path.expanduser(
            "~/yuna/historial/"
        )
        ...
        if (
            os.path.exists(bitacora)
            and os.path.getsize(bitacora) > 0
        ):
